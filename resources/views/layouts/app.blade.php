                    ...
                    <div class="flex items-center gap-4">
                        @auth
                            <div class="relative group">
                                ...
                                <!-- Dropdown Menu -->
                                <div
                                    class="absolute right-0 mt-2 w-48 bg-slate-900 border border-slate-700 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                                    <a href="{{ route('profile.edit') }}"
                                        class="block px-4 py-2 text-sm hover:bg-slate-800 rounded-t-lg">
                                        <i class="fas fa-user-circle mr-2"></i>Profile
                                    </a>
                                    <form method="POST" action="{{ route('logout') }}" class="block">
                                        @csrf
                                        <button type="submit"
                                            class="w-full text-left px-4 py-2 text-sm hover:bg-slate-800 rounded-b-lg">
        ...
        <main class="flex-1 py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @if($message = Session::get('success'))
                    <div
                        class="mb-6 p-4 bg-emerald-500/20 border border-emerald-500/50 rounded-lg text-emerald-200 flex items-center gap-3">
            ...
            <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                    <div>
                        ...
                        <p class="text-slate-400 text-sm">Create stunning, unique images powered by advanced artificial
                            intelligence technology.</p>
                    </div>
                    <div>
                        <h4 class="text-sm font-semibold text-white mb-4">Quick Links</h4>
                        <ul class="space-y-2 text-slate-400 text-sm">
                            <li><a href="{{ route('images.create') }}" class="hover:text-indigo-400 transition">Generate
                                    Images</a></li>
                            <li><a href="{{ route('images.index') }}" class="hover:text-indigo-400 transition">My
                                    Gallery</a></li>

<x-app-layout>
                ...
                <div class="image-card group">
                    ...
                    <div class="relative h-64 bg-slate-800 overflow-hidden">
                        ...
                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-300 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
                            <a href="{{ $image->image_url }}" download class="w-12 h-12 bg-indigo-500 rounded-full flex items-center justify-center hover:bg-indigo-600 transition" title="Download image">
                                <i class="fas fa-download text-white"></i>
                            </a>
                            <button onclick="deleteImage({{ $image->id }})" class="w-12 h-12 bg-red-500 rounded-full flex items-center justify-center hover:bg-red-600 transition" title="Delete image">
                                <i class="fas fa-trash text-white"></i>
                            </button>
                        </div>

                        <!-- Size Badge -->
                        <div class="absolute top-3 right-3 bg-black/60 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-indigo-300">
                            {{ $image->size }}
                        </div>
                    </div>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 backdrop-blur-sm">
        <div class="glass-effect p-8 rounded-lg max-w-md w-full mx-4">
            ...
            <div class="flex gap-3">
                <button onclick="closeDeleteModal()" class="flex-1 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition">
                    Cancel
                </button>
